...
class UsersController {
  static async postNew(request, response) {
    ...
    const emailExists = await dbClient.users.findOne({ email }); // Check if email already exists in DB
    if (emailExists)
      return response.status(400).send({ error: "Already exist" }); // Handle existing email
